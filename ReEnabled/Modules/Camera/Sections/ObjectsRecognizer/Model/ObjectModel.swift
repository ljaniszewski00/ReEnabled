...
class ObjectModel {
    ...
    struct ObjectPrediction: Prediction {
        var classIndex: Int
        var score: Float
        var rect: CGRect
    }
    
    let model = Bundle.main.url(forResource: MLModelFile.YOLOV3.fileName,
                                withExtension: "mlmodelc")
    
    public init() { }
    
    public func computeBoundingBoxes(features: MLMultiArray, scale: Int, scaleIndex: Int) -> [ObjectPrediction] {
        var predictions = [ObjectPrediction]()
        
        let blockSize: Float = Float(416) / Float(scale)
        let gridHeight = scale
        let gridWidth = scale
        let boxesPerCell = 3
        let numClasses = 80
        
        // The 416x416 image is divided into a SCALExSCALE grid. Each of these cells
        // will predict 3 bounding boxes (boxesPerCell). A bounding box consists of
        // five data items: x, y, width, height, and a confidence score. Each grid
        // cell also predicts which class each bounding box belongs to.
        //
        // The "features" array therefore contains (numClasses + 5)*boxesPerCell
        // values for each grid cell, i.e. 255 channels. The total features array
        // contains 255xSCALExSCALE elements.
        ...
        for cy in 0..<gridHeight {
            for cx in 0..<gridWidth {
                for b in 0..<boxesPerCell {
                    ...
                    // For the first bounding box (b=0) we have to read channels 0-84,
                    // for b=1 we have to read channels 85-169, and so on.
                    ...
                    // Skip this loop immediately if confidence is too low to save
                    // calculation
                    ...
                    // 416x416 image space.
                    let scalePow = powf(2.0,Float((4 - scaleIndex) - 1)) // scale pos by 2^i where i is the scale pyramid level
                    let x = (Float(cx) * blockSize + sigmoid(tx)) / scalePow
                    let y = (Float(cy) * blockSize + sigmoid(ty)) / scalePow
                    
                    // The size of the bounding box, tw and th, is predicted relative to
                    // the size of an "anchor" box. Here we also transform the width and
                    // height into the original 416x416 image space.
                    let w = exp(tw) * anchors[scaleIndex - 1][2*b    ] // * blockSize
                    let h = exp(th) * anchors[scaleIndex - 1][2*b + 1] // * blockSize
                    ...
                    if confidenceInClass > confidenceThreshold {
                        ...
                        let prediction = ObjectPrediction(classIndex: detectedClass,
